...
export default function ScriptsPage() {
  ...
  const [genForm, setGenForm] = useState({
    topic: "", genre: "", targetAudience: "", duration: "60", tone: "", keywords: "", projectId: "",
  });
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setGenForm({ topic: "", genre: "", targetAudience: "", duration: "60", tone: "", keywords: "", projectId: "" });
      fetchData();
    } finally {
      setGenerating(false);
    }
                ...
                <CardContent className="p-4">
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                    <div className="flex gap-3 text-xs text-gray-400">
                      <span>画像 {script._count.imageAssets}</span>
                      <span>動画 {script._count.videoAssets}</span>
                    </div>
        ...
        <DialogContent className="max-w-xl">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleGenerate} disabled={!genForm.topic || !genForm.projectId || generating}>
              {generating ? <><Loader2 size={14} className="animate-spin" /> 生成中...</> : <><Sparkles size={14} /> 生成する</>}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
